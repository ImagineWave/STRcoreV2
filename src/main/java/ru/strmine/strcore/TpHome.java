...
public class TpHome implements CommandExecutor {
	...
	public boolean onCommand (CommandSender sender, Command cmd, String label, String[] args) {
		...
		p.teleport(home);
		MessageManager.getManager().msg(sender, MessageManager.MessageType.INFO, "Добро пожаловать домой!");
		}
		if (args.length == 1) {
			...
			if(Bukkit.getOnlinePlayers().contains(Bukkit.getPlayerExact(args[0]))) {
				...
				p.teleport(home);
				MessageManager.getManager().msg(sender, MessageManager.MessageType.INFO, "Вы телепортировались в дом игрока §6" + t.getName());
			...
			p.teleport(home);
			MessageManager.getManager().msg(sender, MessageManager.MessageType.INFO, "Вы телепортировались в дом игрока §6" + args[0] +" §cВнимание, игрок не в сети");
			}
			catch (IllegalArgumentException e) {
				MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Игрок незарегистрирован или не имеет дома");
				return true;
			}
		}
	return true;
