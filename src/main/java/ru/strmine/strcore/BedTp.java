...
public class BedTp implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		try{
				...
				p.teleport(bed);
				MessageManager.getManager().msg(sender, MessageManager.MessageType.INFO, "Добро пожаловать в дом игрока §6" + args[0]);
				for(Player pls : Bukkit.getServer().getOnlinePlayers()) {
					...
					MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Игрока не существует / не имеет кровати");
					return true;
			}
		return false;
	}
