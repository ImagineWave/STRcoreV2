	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length==1) {
			if(sender.hasPermission("str.spawn.others")) {
				...
				t.teleport(spawn);
				MessageManager.getManager().msg(sender, MessageManager.MessageType.GOOD, "Вы телепортировали игрока "+ p.getName()+" на спаун");
				MessageManager.getManager().msg(t, MessageManager.MessageType.GOOD, "Вас телепортировали на спаун");
				return true;
			}
		}
		if (!sender.hasPermission("str.spawn")) {
			...
			MessageManager.getManager().msg(sender, MessageManager.MessageType.GOOD, "Координаты спауна x="+infoX +", z="+infoZ);
	            return true;
	        }
		p.teleport(spawn);
		MessageManager.getManager().msg(p, MessageManager.MessageType.INFO, "Добро пожаловать на спаун");
		return false;
	}
