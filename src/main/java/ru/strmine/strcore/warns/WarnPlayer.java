package ru.strmine.strcore.warns;
...
public class WarnPlayer implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length<2) {
			MessageManager.getManager().msg(sender, MessageManager.MessageType.INFO, "/warn <игрок> <причина> - Выдать варн");
			MessageManager.getManager().msg(sender, MessageManager.MessageType.INFO, "/warn <list/remove> <игрок> - Список варнов игрока/Удалить все варны");
			return true;
		}
		...
		switch (args[0]) {
			case("list"):{
				...
				if(!isPlayerExist(targetName)) {
					MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Игрока "+targetName+" не существует");
					return true;
				}
			...
			default:{
				...
				if(reason.contains("|")) {
					MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Символ §6|§c запрещен");
					return true;
				}
