...
import ru.strmine.strcore.StrPlayer;
...
public class SetPrefix implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length==0){
			if(sender.hasPermission("str.setprefix")) {
				MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "/setprefix <Prefix>");
				return true;
			}
			if(sender.hasPermission("str.setprefix.others")) {
				MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "/setprefix <Игрок> <Prefix>");
				return true;
			}
		}
		if(args.length==1){
		...
		if(args.length==2) {
			...
			setPrefixToGame(p);
			MessageManager.getManager().msg(sender, MessageManager.MessageType.GOOD, "Вы установили префикс §6"+prefix+" §aигроку §6"+target.getName());
			return true;
		}
		return false;
	}
	private void checkPrefixAndBypass(Player p, String prefix) {
		if(p.hasPermission("str.prefix.bypass")) {
			if(isThereShittySymbols(prefix)) {
				MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Никаких §kкринж §cв префиксе");
				return;
			}
			setPrefixToCFG(p,prefix);
			setPrefixToGame(p);
			MessageManager.getManager().msg(p, MessageManager.MessageType.GOOD, "Префикс установлен, перезайдите на сервер");
			return;
		}
	...
	private boolean isPrefixAllowed(Player p, String prefix) {
		if(getPrefixLenth(prefix)>10) {
			MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Длина префикса не более 10 симоволов");
			return false;
		}
