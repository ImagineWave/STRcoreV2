...
public class CreateClan implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length<2) {
			MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "/clancreate <название> <тег>");
			return true;
		}
		String clanName = args[0];
		String clanTag = args[1];
		if((sender.hasPermission("str.clanmember"))&&(!p.isOp())) {
			MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Вы уже состоите в клане");
			return true;
		}
		if(!isClanNameAllowed(clanName)) {
			MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Это имя клана уже занято");
			return true;
		}
	...
	private void createClan(Player p, String name, String tag) {
		...
		MessageManager.getManager().msg(p, MessageManager.MessageType.GOOD, "Клан создан, перезайдите в игру для обновления в TAB");
	}
