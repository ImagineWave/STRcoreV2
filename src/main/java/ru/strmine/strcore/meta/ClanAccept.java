...
public class ClanAccept implements CommandExecutor{
	...
	private void joinClan(Player p) {
		...
		if(!((clanName ==null)||(clanName.equals("0") ))) {
			MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Сначала покиньте ваш клан!");
			return;
		}
		...
		MessageManager.getManager().msg(p, MessageManager.MessageType.GOOD, "Вы вступили в клан §6"+inviteName+"§a перезайдите в игру для обновления в TAB");
	}
