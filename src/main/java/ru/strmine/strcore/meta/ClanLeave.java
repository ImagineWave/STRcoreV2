...
public class ClanLeave implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender.isOp()) {
			MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Сначала используйте /deop "+sender.getName());
			return true;
		}
		...
		if(checkOwner(p)) {
			MessageManager.getManager().msg(sender, MessageManager.MessageType.BAD, "Владелец не может покинуть клан, используйте /clanmodify delete <название>");
			return true;
		}
		leaveClan(p);
		return true;
	...
	private void leaveClan(Player p) {
		...
		MessageManager.getManager().msg(p, MessageManager.MessageType.GOOD, "Вы покинули клан §6"+clanName+"§a перезайдите в игру для обновления в TAB");
		try {
			c.save(clans);
		} catch (IOException e) {
			e.printStackTrace();
