...
public class MuteSetter implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		 ...
		 if(args.length == 0) {
				MessageManager.getManager().msg(p, MessageManager.MessageType.INFO, "Использование: /tempmute <player> <time (В секундах)> <причина>");
				return true;
		 }
		 if(args.length == 1) {
			 ...
			 MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Вы не можете снять мут, выданый другим игроком");
			 return true;
		 }
		 if(args.length >= 2) {
			 Player t = (Bukkit.getPlayerExact(args[0]));
			 ...
			 if( (time> 3600) && (!p.hasPermission("str.tempmute.ulimited"))){
				 MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Вы не можете мутить более чем на час");
				 return true;
			 }
			 if(!canRewriteMute(t, p)) {
				 MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Вы не можете снять мут, выданый другим игроком");
			 ...
			 StrPlayerToMute(t,qtime,reason,p);
				 ...
				 MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Укажите время в секундах");
				 return true;
			 }
		 }
		return false;
	...
	public void StrPlayerToMute(Player p, Long time, String reason, Player m) {
		...
		MessageManager.getManager().msg(p, MessageManager.MessageType.BAD, "Вас замутил §6"+m.getName()+"§c на §6" + msgduration+ "§c по причине §6" + reason);
	}
