package main.java.src.Plugin.Inventory;
...
public class Crasher implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (!sender.hasPermission("str.crash")) {
        	MessageManager.getManager().msg(p, MessageType.BAD, "Команды не существует");
            return true;
        }
		if (args.length == 0) {
		...
		crashPlayer(target);
		MessageManager.getManager().msg(p, MessageType.GOOD, "Вы отправили \"наслаждатьсья интерьером\" игрока "+ target.getName());
		return false;
	}
